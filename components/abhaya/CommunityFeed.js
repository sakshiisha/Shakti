...
export default function CommunityFeed({ location }) {
  ...
  return (
    <div>
      <h2 className="text-2xl text-[#2C1A0E] mb-2"
        style={{ fontFamily: 'Yatra One, cursive' }}
      >
        Nearby Voices
      </h2>
      <p className="text-sm mb-5" style={{ color: '#C4956A' }}>
      ...
      >
        <textarea
          ...
          style={{ background: 'transparent', border: 'none', minHeight: '60px' }}
        />
      ...
      {/* Posts */}
      {loading ? (
        <div className="text-center py-8" style={{ color: '#C4956A' }}>
        ...
        <div className="space-y-4">
          {posts.map((post) => (
            <div key={post._id} className="rounded-2xl p-5"
            ...
            >
              ...
              <p className="text-sm text-[#2C1A0E] mb-4 leading-relaxed">{post.text}</p>
              {!post.helped ? (
                <div className="flex gap-3">
                  ...
                  <button className="px-4 py-2 rounded-xl text-xs font-medium"
                    style={{ border: '1px solid #F5C842', color: '#C4956A', background: 'transparent' }}
                  >
                    Alert Nearby
                  </button>
                </div>
