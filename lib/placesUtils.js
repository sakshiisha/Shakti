// Fallback data — jab API fail ho
const FALLBACK_RESOURCES = [
  { icon: '🏥', label: 'Hospital',       dist: 'Nearby' },
  { icon: '🚔', label: 'Police — 112',   dist: 'Call'   },
  { icon: '💊', label: 'Pharmacy',       dist: 'Nearby' },
]
...
export const getNearbyPlaces = async (lat, lng) => {
  try {
    ...
    if (!res.ok) return FALLBACK_RESOURCES
    ...
    return results.length > 0 ? results : FALLBACK_RESOURCES
